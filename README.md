...
```
/dcp-compress 强力压缩 或 /dcp-purge 激进清理
       │
       ▼
command.execute.before
       │
       ├─ state.pendingManualTrigger = { prompt }   ← 包含用户焦点
       ├─ output.parts = [{ text: "/dcp-compress" 或 "/dcp-purge" }]
       └─ return (不 throw)
       │
       ▼
messages.transform → applyPendingManualTrigger()
       ...
       │
       ▼
LLM 收到 prompt → 调用 compress 或 purge 工具
```

### 压缩管道（compress / purge 共用）

```
prepareSession()                                   ← 两工具共用
  ├─ manual mode 检查
  ├─ 获取对话消息
...
applyCompressionState()                            ← 两工具共用
  └─ 注册压缩块 → 更新状态

finalizeSession()                                  ← 两工具共用
  └─ 持久化状态 + 发送通知
```
...
```jsonc
{
    ...
    // 按模型覆盖上限，键名格式 provider/model
    "modelMaxLimits": {},

    // 按模型覆盖下限
    "modelMinLimits": {},

    // 摘要 token 是否计入 maxContextLimit（true = 不计入，等效扩展上下文窗口）
    "summaryBuffer": true,

    // 通知显示：off 关闭 / minimal 简洁 / detailed 详细
    "pruneNotification": "detailed",

    // 通知位置：chat 对话内 / toast 弹窗
    ...
    // ============================================================
    // 上下文压缩（/dcp-compress / compress 工具）
    // 受 protectedTools / protectTags / protectUserMessages 限制
    // ============================================================
    ...
    // ============================================================
    // 激进压缩（/dcp-purge / purge 工具）
    // 无任何内容保护限制——替换所选范围内所有内容
    // ============================================================
...
| `compress.permission`                 | compress | compress nudge / compress 工具         |
| `purge.autonomous`                    | purge    | purge nudge                            |
| `purge.nudgeFrequency`                | purge    | purge nudge                            |
| `manualMode.enabled`                  | compress | compress nudge（purge nudge 不受影响） |

**说明：**

- `lang`、`maxContextLimit`、`minContextLimit`、`summaryBuffer` 从 `compress` 下提升到配置根级，compress 和 purge 共用
- `compress.permission` **只控制 compress nudge 注入和 compress 工具注册**，不影响 purge
- `purge.autonomous` 和 `purge.nudgeFrequency` 完全独立于 compress，仅控制 purge nudge
- `manualMode.enabled` **只阻塞 compress nudge**，不阻塞 purge nudge
