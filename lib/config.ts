...
export function getInvalidConfigKeys(
...
export function validateConfigTypes(
  ...
  if (
    ...
    errors.push({
      key: "autoUpdate",
      expected: "boolean",
      actual: typeof config.autoUpdate,
    });
  }
  ...
  if (config.protectedFilePatterns !== undefined) {
    if (!Array.isArray(config.protectedFilePatterns)) {
      ...
      !config.protectedFilePatterns.every((v: unknown) => typeof v === "string")
    ) {
      errors.push({
        key: "protectedFilePatterns",
        expected: "string[]",
  ...
  if (commands !== undefined) {
    if (
      ...
      if (
        commands.protectedTools !== undefined &&
        !Array.isArray(commands.protectedTools)
      ) {
        errors.push({
  ...
  if (manualMode !== undefined) {
    if (
      typeof manualMode !== "object" ||
      manualMode === null ||
      Array.isArray(manualMode)
    ) {
      errors.push({
        key: "manualMode",
        expected: "object",
      ...
      if (
        manualMode.enabled !== undefined &&
        typeof manualMode.enabled !== "boolean"
      ) {
        errors.push({
  ...
  if (compress !== undefined) {
    if (
      ...
      if (
        compress.protectedTools !== undefined &&
        !Array.isArray(compress.protectedTools)
      ) {
        errors.push({
      ...
      if (
        compress.protectTags !== undefined &&
        typeof compress.protectTags !== "boolean"
      ) {
        errors.push({
      ...
      if (
        compress.permission !== undefined &&
        !validValues.includes(compress.permission)
      ) {
        errors.push({
...
function showConfigWarnings(
  ...
  if (invalidKeys.length > 0) {
    ...
    const suffix =
      invalidKeys.length > 3 ? ` (+${invalidKeys.length - 3} more)` : "";
...
function createDefaultConfig(): void {
  ...
  const configContent = JSON.stringify(output, null, 2) + "\n";
...
function loadConfigFile(configPath: string): ConfigLoadResult {
  ...
  try {
    ...
    return {
      data: null,
      parseError: error.message || "Failed to parse config",
    };
  }
}
...
function mergeCompress(
  ...
  return {
    ...
    protectUserMessages:
      override.protectUserMessages ?? base.protectUserMessages,
  };
}
...
function mergeManualMode(
  ...
  return {
    enabled: override.enabled ?? base.enabled,
    automaticStrategies:
      override.automaticStrategies ?? base.automaticStrategies,
  };
}
...
function mergeLayer(
  ...
  return {
    ...
    pruneNotificationType:
      data.pruneNotificationType ?? config.pruneNotificationType,
